...
async function extractTextFromPDFsInFolder(folderPath, outputFolder, quality) {
  ...
  for (const file of files) {
    ...
    for (const image of images) {
      ...
      progress++;
      console.log(`${progress} de ${files.length} analisados | ${filesWithError.length} falhas`);
...
extractTextFromPDFsInFolder(path.resolve(inputDir), path.resolve(outputDir));
